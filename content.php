...
<?php if(have_posts()): while(have_posts()): the_post();?>
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h5>
              <em>
                <span class="text-muted author"><?php _e('By', 'ageno-dwt'); echo " "; the_author() ?>,</span>
                <time  class="text-muted" datetime="<?php the_time('d-m-Y')?>"><?php the_time('jS F Y') ?></time>
              </em>
            </h5>
        </header>
        <section>
            <div style="width: 100px; height: 100px; overflow: hidden;"><?php the_post_thumbnail(); ?></div>
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'ageno-dwt' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'ageno-dwt' ) ); ?>
        </section>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="fa fa-folder-open-o"></i>&nbsp; <?php _e('Category', 'ageno-dwt'); ?>: <?php the_category(', ') ?><br/>
                <i class="fa fa-comment-o"></i>&nbsp; <?php _e('Comments', 'ageno-dwt'); ?>: <?php comments_popup_link(__('None', 'ageno-dwt'), '1', '%'); ?>
            </p>
        </footer>
    </article>
<?php endwhile; ?>
...
<?php if ( function_exists('b4st_pagination') ) { b4st_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="fa fa-arrow-left"></i> ' . __('Previous', 'ageno-dwt')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'ageno-dwt') . ' <i class="fa fa-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
